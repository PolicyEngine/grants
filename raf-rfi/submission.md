...
Government users include the Joint Economic Committee, the UK Cabinet Office and HM Treasury, and the New York State Legislature. We are an [NSF POSE Phase I awardee](https://www.nsf.gov/awardsearch/showAward?AWD_ID=2229069) for open-source ecosystem development. Our calculations are validated against [NBER's TAXSIM](https://taxsim.nber.org/) (with an MOU and advisory relationship with Dan Feenberg, TAXSIM's creator) and the [Atlanta Fed Policy Rules Database](https://www.atlantafed.org/economic-mobility-and-resilience/advancing-careers-for-low-income-families/policy-rules-database) for benefit program parameters.

Our platform already does what the RFI describes: we scan statutes and regulations, compile them into executable logic, and surface inconsistencies, gaps, and complexity—including the conflicting rules that are a central focus of RAF's work. We use agentic AI workflows to translate statutory text into validated Python logic. We propose extending this capability into a comprehensive diagnostic and reform toolkit for state governments.
...
Our core stack consists of [policyengine-core](https://github.com/PolicyEngine/policyengine-core), a Python-based declarative policy specification engine derived from [OpenFisca](https://openfisca.org/)—the rules-as-code framework created and used by the French government. Rules are encoded with embedded metadata, citations, and period-aware logic. Time-varying policy values (rates, thresholds, brackets) are stored in YAML files separately from formulas, with full legislative provenance. We perform compile-time analysis of rule relationships through dependency graph construction, enabling automated conflict detection. For statute parsing and code generation, we use Claude and GPT-5 in agentic workflows with human-in-the-loop validation, with continuous automated comparison against TAXSIM and Atlanta Fed benchmarks.
